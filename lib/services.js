...
module.exports = function * services(beyo, options) {
  ...
  if (!primus) {
    if (options.server) {
      module.exports.primus = primus = loadServer(options.server);
    } else {
...
function * loadServices(beyo, servicesPath, options) {
  ...
  primus.on('connection', function (spark) {
    ...
    beyo.logger.log('debug', 'Connection from', spark);
...
function bindSpark(spark) {
  ...
  function cleanup() {
    ...
    for (var i = 0, ilen = keys.length; i < ilen; ++i) {
      bound = binder[keys[i]];
      emitter.remoteListener(bound.event, bound.listener);
    }
