...
function EmitPacket(uid, event, args) {
  if (!(this instanceof EmitPacket)) {
    return new EmitPacket(uid, event, args);
  }
  ...
  this.uid = uid;  // note no UID validation for emit events....
  this.event = event;
  this.args = args || [];
}
...
function packetParser(data) {
  ...
  if (Type === AddListenerPacket) {
    ...
    return new Type(data.uid, data.event, data.args);
  } else {
    throw PacketException('Unknown packet type `{{type}}`', { type: data.type });
  }
}
