+function (global, Primus) {

  var isConnected = false;
  var primusUrl = undefined/*URL*/;
  var primusOptions = undefined/*OPTIONS*/;

  var slice = Array.prototype.slice;
  var primus = Primus.connect(primusUrl, primusOptions);
  var binder = {};

  //console.log("LIBRARY", primus);
  ...
  function emitEvent(event) {
    if (isConnected) {
      primus.write(createPacket('event', {
        event: event,
  ...
  if ( typeof define === "function" && define.amd ) {
  	define( "Services", [], function() { return new Services(); });
  } else if (global) {
	  global.Services = new Services();
  }

}(window || (module && module.exports), Primus);
